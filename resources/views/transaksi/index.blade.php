...
@section('content')
    ...
    <div class=" d-flex justify-content-between col-xl-8 mb-3 align-items-center">
        <div>
            <a href="javascript:void(0)" id="createTransaksi" class="btn btn-primary d-inline"><iconify-icon
                    icon="ic:baseline-plus"></iconify-icon> Tambah Transaksi</a>
        </div>

        <div class="">
            <label for="filterKode">Filter Berdasarkan Code </label>
            <select class="form-select" id="filterKode" aria-label="Default select example">
                <option value="all" selected>Semua Kode</option>
...
@push('script')
    ...
    <script type="text/javascript">
        $(function() {
            ...
            // function change currency to int
            function convertCurrencyToInt(value) {
                let cleanedValue = String(value).replace(/[^0-9]/g, '');
                cleanedValue = cleanedValue.trim() === '' ? 0 : cleanedValue;
            ...
            $('#saveBtn').click(function(e) {
                ...
                let debit = convertCurrencyToInt(document.getElementById('debit').value);
                let credit = convertCurrencyToInt(document.getElementById('credit').value);

                let formData = $('#transaksi').serializeArray();
                ...
                $.ajax({
                    ...
                    error: function(response) {
                        ...
                        if ((respArr.desc && respArr.desc.length > 0) || (respArr.tanggal &&
                                respArr.tanggal.length > 0) || respArr.code_coa_id && respArr
                            .code_coa_id.length > 0) {
